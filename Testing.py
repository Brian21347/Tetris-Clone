...
class TetrisBlock(pygame.sprite.Sprite):
    def __init__(self, which_block, pos):
        super().__init__()
        self.image = pygame.transform.scale(pygame.image.load(f'Tetris block {which_block} (0 turn).png'),
            ...
            if turn == 'l':  # turn
                try:
                    self.image = pygame.transform.scale(
                        pygame.image.load(f'Tetris block {self.block} ({self.turn + 1} turn).png'),
                        i_size_change(self.block, self.turn + 1))
                    to_be = self.turn + 1
                except FileNotFoundError:
                    self.image = pygame.transform.scale(pygame.image.load(f'Tetris block {self.block} (0 turn).png'),
                ...
                try:
                    ...
                    if self.block in [1, 2, 5]:
                        ...
                        multi = 0
                    self.image = pygame.transform.scale(
                        pygame.image.load(f'Tetris block {self.block} ({multi} turn).png'),
    ...
    def collision_check(self, direction, turn, x, y, to_be):
        if pygame.sprite.spritecollide(the_block.sprite, all_stopped, False, pygame.sprite.collide_mask) or \
            ...
            if turn:
                self.image = pygame.transform.scale(
                    pygame.image.load(f'Tetris block {self.block} ({self.turn} turn).png'),
    ...
    def splitting(self):  # making a sprite into multiple single block sized sprite that can be removed
        ...
        for _ in L[b - 1][t]:
            obstacle = Obstacle(0, 0, _, image_path=f'Tetris block {b}.png')
