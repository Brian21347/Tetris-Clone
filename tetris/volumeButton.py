...
class VolumeButton:
    ...
    def update(self, event: pygame.event.EventType):
        ...
        if (
            self.position[0] < x - VOLUME_BUTTON_BAR_OFFSET < end_x_position
        ...
        if pygame.mouse.get_pressed()[0]:
            ...
            if (
                self.position[0] + self.size[0] < x - VOLUME_BUTTON_BAR_OFFSET < end_x_position
                and self.position[1] < y < self.position[1] + self.size[1]
            ):

                self.volume = (
                    x - self.position[0] - self.size[0] - VOLUME_BUTTON_BAR_OFFSET
                ) / self.expand_rect[0]
    ...
    def draw(self):
        ...
        if self.fill_percent < self.volume:
            return
        pygame.draw.circle(
            self.screen,
            "black",
